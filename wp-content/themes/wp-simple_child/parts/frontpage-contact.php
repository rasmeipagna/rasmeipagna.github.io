...
<?php } else if (nimbus_get_option('contact-toggle') == '3') {
    ...
    <section id="<?php if (nimbus_get_option('fp-contact-slug')=='') {echo "contact";} else {echo nimbus_get_option('fp-contact-slug');} ?>" class="frontpage-row frontpage-contact preview parallax_active" data-parallax='scroll' data-image-src='<?php echo get_template_directory_uri(); ?>/images/preview/field.jpg' style='background: transparent;padding:220px 0 200px;background: rgba(0, 0, 0, 0.3);'>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="contact-title h1">See You Soon</div>
                    <!-- <div class="contact-sub-title h4">Let us know what you are thinking.</div> -->
                    <?php 
                    if(isset($_POST['submitted'])) { 
                        if(isset($_POST['myname']) && isset($_POST['email'])) {
                            if((trim($_POST['myname']) != "" ) && (trim($_POST['email']) != "" )) { ?>
                                <p class="bg-success"><?php echo _e('Merci de m\'avoir contacté, je vous répondrai dans les plus brefs délais !', 'nimbus_venture'); ?></p>
                            <?php } else { ?>
                                <p class="bg-danger"><?php echo _e('Veuillez entrer votre nom et votre adresse e-mail', 'nimbus_venture'); ?></p>
                            <?php 
                            }
                        }
                    }
                    ...
                    <!-- <div class="row">
                        <div class="col-md-8 col-md-offset-2">
                             <form class="contact-form" name="contact-form" method="post" action="mailto:hello@rasmeipagna.com">
                                <div class="row">
                                    <div class="col-sm-4">
                                        ...
                                        <div class="form-group"><input data-sr="wait 0.3s, enter left and move 50px after 1s" type="text" id="email" name="email" placeholder="Email*" class="form-control input-lg" /></div>
                                        <div class="form-group"><input data-sr="wait 0.3s, enter left and move 50px after 1s" type="text" id="website" name="website" placeholder="Site web" class="form-control input-lg" /></div>
                                    </div>
                                    <div class="col-sm-8">
                                        <textarea data-sr="wait 0.3s, enter right and move 50px after 1s" id="comments" name="comments" class="form-control input-lg" rows="6" cols="" placeholder="Votre message"></textarea>
                                    </div>
                                </div>
                                <br />
                                <div class="text-center">
                                    <input type="hidden" name="scrolltoform" value="<?php if (nimbus_get_option('fp-contact-slug')=='') {echo "contact";} else {echo nimbus_get_option('fp-contact-slug');} ?>">
                                    <input type="hidden" name="submitted" id="submitted" value="true" />
                                    <input data-sr="wait 0.3s, enter right and move 50px after 1s" type="submit" name="submit_button" id="submit_button" value="Envoyer" class="contact-submit" />
                                </div>
                            </form>
                        </div>
                    </div> -->
